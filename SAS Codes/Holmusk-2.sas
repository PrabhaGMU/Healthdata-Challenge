...
select min(tot_amount) as min, max(tot_amount) as max
...
/*prep: creating tables for regression analysis*/

/*this table was created for regression purpose to group few variables as continous*/
proc sql;
...
proc sql;
create table Health.Healthdata_regression_only_Final as
select gender,race, Weight1,Height1,Final_cost,LOS_1,Age1,medical_history_1, medical_history_2, 
...
/*grouping varible race as continous variable*/
...
/*Final table used for regression analysis Dependent variable: Final_cost
changing the data type from character to numeric for 2 variables below*/
...
proc print data= regression_only_Final (obs=10);
run;
...
/* Descriptive Analysis- inserted different variables to 
form a table with all variables*/

proc freq data=Health.Healthdata_descriptive_only;
tables medical_history_7*race/ norow nocol;
run;

/*visualization*/

proc univariate data=Health.Healthdata_descriptive_only normal plot;
var LOS;
run;


/*Bar Chart:change the variables in group and check the graph*/

PROC SGPLOT DATA =Health.Healthdata_descriptive_only;
VBAR Final_cost / GROUP =LOS;
...
proc template;
  define statgraph histogram;
    begingraph;
      entrytitle "Total cost by RACE";
      layout overlay /;
        histogram tot_amount / name="histogram" group=Race;
        discretelegend "histogram" / title="RACE";
      endlayout;
    endgraph;
  end;
run;
...
proc sgrender template=histogram data=Health.Final_table_Descriptive_stat;
run;
...
proc sgplot data=Health.Final_table_Descriptive_stat;
   scatter y=tot_amount x=LOS;
run;


/*Pearson Correlation for dependent continous varible and independent continous variable*/


PROC CORR DATA=Health.Final_table_Descriptive_stat PLOTS=SCATTER(NVAR=all);
   VAR tot_amount LOS;
RUN;

/*ttest for dependent continous varible and independent categorical variable*/


PROC TTEST DATA=regression_only_Final ALPHA=.05;
   VAR Final_cost;
   CLASS Gender1;
RUN;
...
/*Multiple Linear regression Model*/
 ...
 model Final_cost=LOS_1 race1 Gender1 Age1 height1 weight1 medical_history_1 medical_history_2	medical_history_3N	medical_history_4	medical_history_5 medical_history_6	medical_history_7 	
	 symptom_1 symptom_2 symptom_3 symptom_4 symptom_5/ SOLUTION CLPARM;
